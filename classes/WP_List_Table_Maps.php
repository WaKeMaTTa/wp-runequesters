 ...
 */
class WP_List_Table_Maps extends WP_List_Table {
	...
	function column_default($item, $column_name){
		switch($column_name){
			case 'col_author':
				return $item["map_author"];
			case 'col_num_points':
				return $item["map_num_points"];
			default:
	...
	function column_col_title($item) {
		...
		//Build row actions
		$actions["points"] = sprintf('<a href="?page=%s&action=%s&map=%s">%s</a>',
			$_REQUEST['page'],
			'points',
			$item['map_id'],
		...
		if ( current_user_can( 'manage_options' ) )
			$actions["delete"] = sprintf('<a href="?page=%s&action=%s&map=%s">%s</a>',
				$_REQUEST['page'],
				'delete',
				$item['map_id'],
		...
		return sprintf('<span style="color:silver">[ID %2$s]</span> %1$s %3$s',
			/*$1%s*/ $item['map_title'],
			/*$2%s*/ $item['map_id'],
	...
	function column_col_description($item){		
		...
		return wp_trim_words( $item["map_description"], 30, '&hellip;' );
	}

	function column_col_author($item){
		//get name user
		$user = get_userdata( $item["map_author"] );

		if ($user === false) {
			$return = '<span style="color: #a00;">';
			$return .= sprintf( __( 'Author doesn\'t exist [ID %s]' , WPRQ_TEXTDOMAIN ),
				$item["map_author"]
			);
			$return .= '</span>';
			return $return;
	...
	function get_columns(){
		$columns = array(
			...
			'col_num_points'	=> __( 'Nº Points', WPRQ_TEXTDOMAIN ),
	...
	function get_sortable_columns() {
		$sortable_columns = array(
			'col_title'	 => array('map_title', false),	 //true means it's already sorted
		);
	...
	*/
	...
	function prepare_items() {
		 ...
		 */
		$query = "SELECT * FROM " . $wpdb->prefix . WPRQ_NAME . "_maps";
