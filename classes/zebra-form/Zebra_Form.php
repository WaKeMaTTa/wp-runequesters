    ...
    function render($template = '', $return = false, $variables = '')
    {
            ...
            // if
                ...
                if (isset($this->controls[$attributes['for']]->rules['required']))
                    ...
                    if (
                        ...
                        if (strpos($this->controls[$this->master_labels[$ctrl_attributes['name']]['control']]->attributes['label'], ' <span class="description">(' . __( 'required' , WPRQ_TEXTDOMAIN ) . ')</span>') === false)

                            // attach the asterisk to the "master" label instead rather than to the current label
                            $this->controls[$this->master_labels[$ctrl_attributes['name']]['control']]->set_attributes(array('label' => $this->controls[$this->master_labels[$ctrl_attributes['name']]['control']]->attributes['label'] . ' <span class="description">(' . __( 'required' , WPRQ_TEXTDOMAIN ) . ')</span>' ));

                    // otherwise
                    } else

                        // attach the asterisk to the current label
                        $this->controls[$key]->set_attributes(array('label' => $attributes['label'] . ' <span class="description">(' . __( 'required' , WPRQ_TEXTDOMAIN ) . ')</span>'));
        ...
        if (!empty($this->errors))
            ...
            foreach ($this->errors as $error_block => $error_messages) {
                ...
                $this->errors[$error_block] = '<div class="alert ' . $error_block . ' fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>' . $content . '</div>';
                    ...
                    foreach ($controls as $control) {
                        ...
                        // if not an array of controls
                            ...
                            if (array_key_exists('required', $this->controls[$control]->rules) && preg_match('/\binside\b/', $this->controls[$control]->attributes['class'])) {
                                ...
                                $contents .= $this->controls[$control]->toHTML() . ' <span class="description">(' . __( 'required' , WPRQ_TEXTDOMAIN ) . ')</span>';
            ...
            foreach ($this->controls as $control) {
                ...
                if (!isset($attributes['inside']))
                    ...
                    if (array_key_exists('required', $control->rules) && preg_match('/\binside\b/', $control->attributes['class'])) {
                        ...
                        $controls[$attributes['id']] = $control->toHTML() . ' <span class="description">(' . __( 'required' , WPRQ_TEXTDOMAIN ) . ')</span>';
